...
class Game
{
  ...
  public void update()
  {
    ...
    if (gameOverP2 != false) {
      clearBoard();
      text("GAME OVER: SPILLER 1 VANDT", 50, 50);
      noLoop();
    }
    if (gameOverP1 != false) {
      clearBoard();
      text("GAME OVER: SPILLER 2 VANDT", 50, 50);
      noLoop();
    }
  }
  ...
  private void checkForCollisionsP2()
  {
    ...
    for (int i = 0; i < enemies.length; ++i)
    {
      if (enemies[i].getX() == player2.getX() && enemies[i].getY() == player2.getY()&& game.getPlayer2Life()>-0)
      {
        //We have a collision
        --player2Life;
        if (player2Life<=0) {
  ...
  private void checkForFoodCollisionsP2() {
    for (int j = 0; j < enemies.length; ++j)
    {
      if (food[j].getX() == player2.getX() && food[j].getY() == player2.getY()&& game.getPlayer2Life()<100)
      {
        ++player2Life;
        food[j].x= (int) random(width-1);
        food[j].y = (int) random(width-1);
